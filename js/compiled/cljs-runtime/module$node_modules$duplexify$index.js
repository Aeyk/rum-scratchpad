shadow$provide.module$node_modules$duplexify$index=function(global,require,module,exports){var process=require("module$node_modules$process$browser");global=require("module$node_modules$buffer$index").Buffer;var stream=require("module$node_modules$readable_stream$readable_browser"),eos=require("module$node_modules$end_of_stream$index");exports=require("module$node_modules$inherits$inherits_browser");var shift=require("module$node_modules$stream_shift$index"),SIGNAL_FLUSH=global.from&&global.from!==
Uint8Array.from?global.from([0]):new global([0]),onuncork=function(self,fn){if(self._corked)self.once("uncork",fn);else fn()},destroyer=function(self,end){return function(err){err?self._autoDestroy&&self.destroy("premature close"===err.message?null:err):end&&!self._ended&&self.end()}},end$jscomp$0=function(ws,fn){if(!ws||ws._writableState&&ws._writableState.finished)return fn();if(ws._writableState)return ws.end(fn);ws.end();fn()},toStreams2=function(rs){return(new stream.Readable({objectMode:!0,
...
enc,cb){if(this.destroyed)return cb();if(this._corked)return onuncork(this,this._write.bind(this,data,enc,cb));if(data===SIGNAL_FLUSH)return this._finish(cb);if(!this._writable)return cb();!1===this._writable.write(data)?this._ondrain=cb:cb()};Duplexify.prototype._finish=function(cb){var self=this;this.emit("preend");onuncork(this,function(){end$jscomp$0(self._forwardEnd&&self._writable,function(){!1===self._writableState.prefinished&&(self._writableState.prefinished=!0);self.emit("prefinish");onuncork(self,
cb)})})};Duplexify.prototype.end=function(data,enc,cb){if("function"===typeof data)return this.end(null,null,data);if("function"===typeof enc)return this.end(data,null,enc);this._ended=!0;data&&this.write(data);this._writableState.ending||this.write(SIGNAL_FLUSH);return stream.Writable.prototype.end.call(this,cb)};module.exports=Duplexify}
